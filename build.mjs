...
import { acquireInputs, verifyInstalledInputs } from './scripts/registry-inputs.mjs';
import { verifyInputs } from './scripts/inputs.mjs';
const root=path.dirname(fileURLToPath(import.meta.url));
// Invalidate old successful outputs before any trust-sensitive work.
for(const name of ['dist','references','SKILL.md','out']) await rm(path.join(root,name),{recursive:true,force:true});
const args=process.argv.slice(2);
assert.ok(args.length===0 || (args.length===2 && args[0]==='--inputs'),'usage: npm run build [-- --inputs inputs/inputs.json]');
const recordPath=args.length ? path.resolve(args[1]) : (await acquireInputs()).recordPath;
const inputs=await verifyInputs(recordPath);
